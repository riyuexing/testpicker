...
class CouponcodesController extends Controller
{
    ...
    public function create()
    {
      if(!checkRole(getUserGrade(2)))
      {
    	...
    	$data['record']         	= FALSE;
    	$data['active_class']       = 'coupons';
        $data['categories']           =array('exam'=>getPhrase('Quizzes'), 'combo'=>getPhrase('Examseries'), 'lms'=> getPhrase('LMS'), 'subscription'=> getPhrase('Subscription'));
    ...
    public function validateCoupon(Request $request)
    {
        ...
        elseif ($item_type=='subscription') {
            $item = App\Subscription::where('slug','=',$request->item_name)->first();
        }
