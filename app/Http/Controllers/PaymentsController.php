...
public function approvePayment(Payment $payment_record,Request $request ,$iscoupon_zero = 0)
{
        ...
        if ($payment_record->plan_type == 'subscription') {

          $item_model = new Subscription();
        }
        ...
        //checking if there is any active subscription

        if ($user->subscription_ends_at == null) {
            $payment_record->start_date = date('Y-m-d');
            $payment_record->end_date   = date('Y-m-d', strtotime($daysToAdd));
        }else{
            $current_subscription =$user->subscription_ends_at;
            $new_subscription = $current_subscription->addDays($item_details->validity);
            $payment_record->start_date = $user->subscription_ends_at;
            $payment_record->end_date   = $new_subscription;
        }
        ...
        if ($user->subscription_ends_at == null) {
           $user->subscription_ends_at = date('Y-m-d', strtotime($daysToAdd));
           $user->save();
        }else{
          $current_subscription =$user->subscription_ends_at;
          $new_subscription = $current_subscription->addDays($item_details->validity);
  ...
  public function razorpaySuccess(Request $request)
  {
        ...
        if(count($input)  && !empty($input['razorpay_payment_id'])) {
            ...
            try {
                ...
                if ($user->subscription_ends_at == null) {
                    ...
                    $current_subscription = $user->subscription_ends_at;
                    $new_subscription = $current_subscription->addDays($item->validity);
                    $payment_record->start_date = $user->subscription_ends_at;
                    $payment_record->end_date   = $new_subscription;
                  }
   ...
   public function showSubscriptionList(){
        ...
        if ($user->phone_verify != 1) {
          flash('Error','Please Verify Mobile Number', 'overlay');
          $link = 'users/edit/'.$user->slug;
        ...
        $data['layout']             = getLayout();
        $view_name = getTheme().'::payments.subscription';
        return view($view_name, $data);
   }
